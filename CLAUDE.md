...
Wszelkie zmiany wprowadzaj **wyłącznie lokalnie**. Nie pushuj do repozytorium GitHub bez wyraźnej prośby użytkownika. Commit i push tylko na polecenie.
...
css/components/           ← navbar, footer, buttons, cards, badge, cookie-fab
...
- **Nowa podstrona:** skopiuj strukturę `<head>` z istniejącej strony (zawiera już bloki cookie consent [1–5]), dodaj `#navbar-placeholder` i `#footer-placeholder`, załaduj `js/loader.js`, `js/navbar.js` i `/js/cookie-consent.js`. Pamiętaj o linku `css/components/cookie-fab.css` — bez niego przycisk ustawień cookies nie będzie widoczny dla powracającego użytkownika.
